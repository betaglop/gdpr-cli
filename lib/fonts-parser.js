...
class FontsParser {
  ...
  getFonts() {
    ...
    if (this.hasWixFonts()) this.fonts.wix = this.getWixFonts();
  ...
  hasWixFonts() {
    ...
    this.css['inline'].forEach(el => {
      if (el.indexOf('//static.wixstatic.com/ufonts') !== -1) wix = true;
    });
    
    this.css.forEach(el => {
      if (el.url.href.indexOf('//static.wixstatic.com/ufonts') !== -1) wix = true;
    });
  ...
  getWixFonts() {
    ...
    var _getFont = function(fp, content, url) {
      let fontfaces = content.match(/\@font-face\s?{([^}]*)/gm);
      if (fontfaces !== null) {
        fontfaces.forEach(ff => {
          let font = {};
          font.font = fp.getFontName(ff);
          if (fp.getFontStyle(ff) !== '') font.style = fp.getFontStyle(ff);
          if (fp.getFontWeight(ff) !== '') font.weight = fp.getFontWeight(ff);
          font.url = !url && fp.getFontUrl(ff) !== '' ? fp.getFontUrl(ff) : url;
          font.type = 'wixfont';
          fonts.push(font);
        });
      }
    ...
    this.css['inline'].forEach(el => {
      if (el.indexOf('//static.wixstatic.com/ufonts') !== -1) {
        _getFont(this, el);
      }
    });
    
    this.css.forEach(el => {
      if (el.url.href.indexOf('//static.wixstatic.com/ufonts') !== -1) {
        _getFont(this, el.content, el.url);
      }
    });
  ...
  /**
   * Gets the src/url of the font
   * @param {string} fontface 
   * @class FontsParser
   */
  getFontUrl(fontface) {
    let style = '';
    if (fontface.match(/src\:\s?url\(['"]?([^'"\),;]*)/) !== null) {
      style = fontface.match(/src\:\s?url\(['"]?([^'"\),;]*)/)[1];
    }
    return style;
  }
