...
class Tasks {
      ...
      for ( var i in this.data.analytics ) {
        if ( this.data.analytics[i].url ) {
          ...
          this.loadExternalItem(this.data.analytics[i]).then(res => {
            ...
            if (res.headers['set-cookie'] && typeof res.headers['set-cookie'] == 'object') {
  ...
  getVideosInformation() {
    ...
    if ( videos.hasDailymotion(this.data.html) ) {
      vids.push('Dailymotion');
    }

    if ( vids.length == 0 ) {
      this.ui.listitem('Free', 'from any external videos');
    }
    vids.forEach(vid => {
      this.ui.listitem(vid, 'is present');
    });
